...
def train_keras():
    ...
    callbacks = [keras.callbacks.ModelCheckpoint("xception_save_at_{epoch}.h5"), ]
    ...
    if save_model_flag:
        file_model_fire = 'Output/Models/model_trans_fire'
        model.save(file_model_fire)
    if Config_classification.get('TrainingPlot'):
        plot_training(trans_fire, 'KerasModel', layers_len)
